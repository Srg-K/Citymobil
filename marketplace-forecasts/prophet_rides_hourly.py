...
from sql_load_data import que_rides
...
for city in ['saint_p','moscow']:
    df = pd.DataFrame(exasol.execute(que_rides.format(city_config[str(city)+'_locality'],city_config[str(city)+'_tarif'])))
    df.columns = ['ds', 'y']
    ...
    m = Prophet(
        ...
        changepoint_prior_scale=0.0055,
        seasonality_prior_scale=10,
        seasonality_mode='multiplicative',
        interval_width=0.95
    ).fit(df)
    future = m.make_future_dataframe(periods=169, freq='H')
    ...
    result.columns = ['date_hour', 'rides', 'rides_lower', 'rides_upper', 'created_date', 'version','locality_id']
    # print(result.head())
    mysql.insert_data_frame("forecast_rides_hourly", result)
